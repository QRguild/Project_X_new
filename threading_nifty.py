...
# instrument_tokens = list(weights.keys())  # Example tokens, replace with your 50 Nifty 50 tokens
...
#                     self.index_value = (weighted_sum/self.latest_prices[256265])
#                     print(total_weight,weighted_sum)
#                 logging.info("Current Weighted Nifty Index Value: %.2f", self.index_value)
...
class WeightedNiftyIndexVWAP(Thread):
    def __init__(self, api_key, access_token):
        self.kws = KiteTicker(api_key, access_token)
        self.weights = {6401: 0.64, 3861249: 0.92, 40193: 0.63, 60417: 1.3, 1510401: 3.03, 4267265: 1.18, 81153: 1.86, 4268801: 0.93, 98049: 0.88, 134657: 0.62, 2714625: 3.95, 140033: 0.64, 177665: 0.78, 5215745: 1.0, 225537: 0.71, 232961: 0.6, 315393: 0.9, 1850625: 1.64, 341249: 11.34, 119553: 0.66, 345089: 0.64, 348929: 0.95, 356865: 2.27, 1270529: 7.74, 424961: 4.16, 1346049: 0.82, 408065: 5.82, 3001089: 0.85, 492033: 2.35, 2939649: 3.73, 519937: 2.38, 2815745: 1.5, 2977281: 1.81, 4598529: 0.83, 633601: 1.0, 3834113: 1.38, 738561: 8.64, 5582849: 0.71, 1102337: 0.86, 779521: 2.62, 857857: 1.79, 2953217: 3.76, 878593: 0.67, 884737: 1.78, 895745: 1.21, 3465729: 0.86, 897537: 1.37, 502785: 1.45, 2952193: 1.16, 969473: 0.66,256265:0}
            ...
            try:
                with self.lock:
                    weighted_sum = sum(self.latest_prices[token] * self.weights[token]
                                       for token in self.instrument_tokens)
                    total_weight = sum(self.weights.values())
                    self.divisor = weighted_sum/self.latest_prices[256265] if self.latest_prices[256265] > 0 else 0
    ...
    def initialise_csv(self):
        self.filename = 'nifty_vwap_'+time.strftime("%Y%m%d")+'.csv'
...
weighted_nifty_index_vwap = WeightedNiftyIndexVWAP(api_key, access_token)
